...
type EtcdSubscriptionDAL struct {
	Mutex sync.Mutex
}

func (dal *EtcdSubscriptionDAL) GetSubscriptions(request common.DALRequest, username string, result chan *subscriptions.GetSubscriptionsResponse) {
	dal.Mutex.Lock()
...
func (dal *EtcdSubscriptionDAL) Subscribe(request common.DALRequest, subscriber string, publisher string, result chan bool) {
	dal.Mutex.Lock()
	defer dal.Mutex.Unlock()
...
func (dal *EtcdSubscriptionDAL) Unsubscribe(request common.DALRequest, subscriber string, publisher string, result chan bool) {
	dal.Mutex.Lock()
	defer dal.Mutex.Unlock()
	subscriptionDB := getSubscriptionCollection(request)
